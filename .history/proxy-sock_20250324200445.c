...
int get_value(int key, char *value1, int *N_value2, double *V_value2, struct Coord *value3) {
    ...
    // Connect to server and send initial request data (action and key)
    ...
    if ((ret = sendMessage(s, buffer, strlen(buffer) + 1)) != 0) {
        close(s);
        return ERROR_COMMUNICATION;
    }
    ...
    // If successful, get the data values
    if (ret == 0) {
        // Read value1
        if ((ret = readLine(s, value1, 256)) != 0) {
            close(s);
            return ERROR_COMMUNICATION;
        }
