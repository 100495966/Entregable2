...
void * SendResponse(void * sc){
    ...
    switch (action) {
        ...
        case EXIST:
            ...
            if ((ret1 = strtol_handling(buffer, &key)) != 0){
                pthread_exit(ret1);
            }
        ...
        case MODIFY_VALUE:
            ...
            value1 = (char *) malloc(strlen(buffer) + 1);
            ...
            if ((ret1 = strtol_handling(buffer, &value3.x)) != 0){
                pthread_exit(ret1);
            }
            if ((ret1 = readLine(s_local, buffer, 256)) != 0){
                pthread_exit(ret1);
            }
            if ((ret1 = strtol_handling(buffer, &value3.y)) != 0){
                pthread_exit(ret1);
            }
